...
from std_msgs.msg import Int8MultiArray, Float32MultiArray, Bool, Int64
...
class OrionDriver(Node):
    def __init__(self):
        ...
        self.get_logger().info('Orion Hybrid Driver listo para /brazo/joint_cmds como Int64')
    ...
    def pack_joint_cmds(self, seq: int, e1: int, e2: int, e3: int, e4: int) -> int:
        return (
            (self._pack_i8(seq) << 0)  |
            (self._pack_i8(e1)  << 8)  |
            (self._pack_i8(e2)  << 16) |
    ...
    def publish_joint_cmds(self, e1, e2, e3, e4):
        msg = Int64()
        msg.data = int(self.pack_joint_cmds(self.tx_seq, e1, e2, e3, e4))
        self.pub_joint_cmds.publish(msg)

        self.get_logger().info(
            f'mode={self.mode} seq={self.tx_seq} e1={e1} e2={e2} e3={e3} e4={e4} packed={msg.data}'
        )
